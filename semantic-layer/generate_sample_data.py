    ...
    for session_idx in range(num_sessions):
        ...
        for event_idx in range(events_in_session):
            ...
            event_offset_minutes = int(
                (event_idx / events_in_session) * duration_minutes
            )
...
print(
    f"   Avg sessions per user: {len(sessions_df) / events_df['user_id'].nunique():.1f}"
)
...
print(
    f"   - Date range: {users_df['signup_date'].min()} to {users_df['signup_date'].max()}"
)
...
print(
    f"   - Date range: {events_df['event_timestamp'].min()} to {events_df['event_timestamp'].max()}"
)
print(f"   - Unique users with events: {events_df['user_id'].nunique()}")
print(
    f"   - Avg events per user: {len(events_df) / events_df['user_id'].nunique():.1f}"
)
...
print(f"   - Avg events per session: {sessions_df['event_count'].mean():.1f}")
print(
    f"   - Avg sessions per user: {len(sessions_df) / sessions_df['user_id'].nunique():.1f}"
)
