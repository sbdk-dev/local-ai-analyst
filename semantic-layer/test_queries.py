...
# Connect to DuckDB
con = ibis.duckdb.connect("data/analytics.duckdb")
...
print(
    f"Events per user: {event_stats['total_events'].iloc[0] / event_stats['unique_users'].iloc[0]:.1f}"
)
print(
    f"Events per session: {event_stats['total_events'].iloc[0] / event_stats['unique_sessions'].iloc[0]:.1f}"
)
...
feature_usage = (
    events.filter(_.event_type == "feature_use")
    .group_by("feature_name")
    .aggregate(usage_count=_.count(), unique_users=_.user_id.nunique())
...
session_durations = sessions.aggregate(
    total_sessions=_.count(), unique_users=_.user_id.nunique()
).execute()
...
print(
    f"Sessions per user: {session_durations['total_sessions'].iloc[0] / session_durations['unique_users'].iloc[0]:.1f}"
)
...
date_range = events.aggregate(
    min_date=_.event_timestamp.min(), max_date=_.event_timestamp.max()
).execute()

print(
    f"Event data range: {date_range['min_date'].iloc[0]} to {date_range['max_date'].iloc[0]}"
)
...
dau = (
    events.filter(_.event_timestamp.date() == reference_date.date())
...
wau = (
    events.filter(_.event_timestamp >= seven_days_ago)
...
mau = (
    events.filter(_.event_timestamp >= thirty_days_ago)
...
# Stickiness (DAU/MAU)
stickiness = (
    (dau["dau"].iloc[0] / mau["mau"].iloc[0]) * 100 if mau["mau"].iloc[0] > 0 else 0
)
...
user_events = users.join(events, users.user_id == events.user_id).select(
    users.user_id, users.signup_date, events.event_timestamp
)
...
d1_returned = (
    user_events.filter((_.days_since_signup >= 1) & (_.days_since_signup < 2))
...
print(
    f"D1 Retention: {d1_retention:.1f}% ({d1_returned['users_returned'].iloc[0]}/{total_users_for_retention} users returned day 1)"
)

# D7 retention: users who had event 7 days after signup
d7_returned = (
    user_events.filter((_.days_since_signup >= 7) & (_.days_since_signup < 8))
    .aggregate(users_returned=_.user_id.nunique())
    .execute()
)
...
print(
    f"D7 Retention: {d7_retention:.1f}% ({d7_returned['users_returned'].iloc[0]}/{total_users_for_retention} users returned day 7)"
)
...
for industry in top_industries:
    top_feature = feature_by_industry.filter(_.industry == industry).limit(1).execute()
    if not top_feature.empty:
        print(
            f"{industry}: {top_feature['feature_name'].iloc[0]} ({top_feature['usage_count'].iloc[0]} uses)"
        )
